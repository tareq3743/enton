...
class FusedObject:
    ...
    __slots__ = ("label", "confidence", "cameras", "attributes", "count")
...
class Fuser:
    ...
    def fuse(
        ...
        self._entity_memory = {
            k: v for k, v in self._entity_memory.items() 
            if now - v["last_seen"] < self._memory_ttl
        }
        ...
        for label in all_labels:
            cam_queues = {
                cid: sorted([d for d in dets if d.label == label], 
            ...
            while any(cam_queues.values()):
                ...
                for other_cid, other_q in cam_queues.items():
                    if other_cid == best_cid: continue
                    if other_q:
